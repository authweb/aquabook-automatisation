...
const AppointmentDetails = () => {
		...
		const fetchEventData = async () => {
			...
			try {
				const response = await axios.get(
					`http://aqua-book.ru:3306/api/appointments/${currentEventId}`,
				);
	...
	useEffect(() => {
		const fetchClientData = async () => {
			...
			try {
				const response = await axios.get(
					`http://aqua-book.ru:3306/api/clients/${appointment.clients_id}`,
				);
	...
	const handlePayment = async () => {
		try {
			const response = await axios.post(
				`http://aqua-book.ru:3306/api/appointments/${currentEventId}/pay`,
			);
